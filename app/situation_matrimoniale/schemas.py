...
class SituationMatrimonialeCreateModel(BaseModel):
...
class SituationMatrimoniale(BaseModel):
    uid: uuid.UUID
    libelle: str
...
class SituationMatrimonialeUpdateModel(BaseModel):
    uid: uuid.UUID
    libelle: str
    code: str | None = Field(default=None, max_length=255)
    cree_le: datetime | None = Field(default=None)
...
class SituationMatrimonialeResponse(BaseModel):
...
class MessageAllResponse(BaseModel, Generic[T]):
    ...
    message: str
    success: bool = True
    data: T | None | List[SituationMatrimonialeResponse] = None

class MessageResponse(BaseModel, Generic[T]):
    code: int
    message: str
    success: bool = True
    data: T | None | SituationMatrimonialeResponse = None
