...
class FicheValidationCreateModel(BaseModel):
...
class FicheValidation(BaseModel):
    uid: uuid.UUID
    libelle: str
...
class FicheValidationUpdateModel(BaseModel):
    uid: uuid.UUID
    libelle: str
    code: str | None = Field(default=None, max_length=255)
    cree_le: datetime | None = Field(default=None)
...
class FicheValidationResponse(BaseModel):
...
class MessageAllResponse(BaseModel, Generic[T]):
    ...
    message: str
    success: bool = True
    data: T | None | List[FicheValidationResponse] = None

class MessageResponse(BaseModel, Generic[T]):
    code: int
    message: str
    success: bool = True
    data: T | None | FicheValidationResponse = None
