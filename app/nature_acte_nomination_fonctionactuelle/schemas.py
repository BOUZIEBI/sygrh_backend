...
class NatureActeNominationFonctionactuelleCreateModel(BaseModel):
...
class NatureActeNominationFonctionactuelle(BaseModel):
    uid: uuid.UUID
    libelle: str
...
class NatureActeNominationFonctionactuelleUpdateModel(BaseModel):
    uid: uuid.UUID
    libelle: str
    code: str | None = Field(default=None, max_length=255)
    cree_le: datetime | None = Field(default=None)
...
class NatureActeNominationFonctionactuelleResponse(BaseModel):
...
class MessageAllResponse(BaseModel, Generic[T]):
    ...
    message: str
    success: bool = True
    data: T | None | List[NatureActeNominationFonctionactuelleResponse] = None

class MessageResponse(BaseModel, Generic[T]):
    code: int
    message: str
    success: bool = True
    data: T | None | NatureActeNominationFonctionactuelleResponse = None
