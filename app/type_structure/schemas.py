...
class TypeStructureCreateModel(BaseModel):
...
class TypeStructureUpdateModel(BaseModel):
    uid: uuid.UUID
    libelle: str
    code: str | None = Field(default=None, max_length=255)
    cree_le: datetime | None = Field(default=None)
...
class MessageAllResponse(BaseModel, Generic[T]):
    ...
    message: str
    success: bool = True
    data: T | None | List[TypeStructureResponse] = None
...
class MessageResponse(BaseModel, Generic[T]):
    ...
    message: str
    success: bool = True
    data: T | None | TypeStructureResponse = None
