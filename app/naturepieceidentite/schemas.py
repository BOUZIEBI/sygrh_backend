...
class NaturepieceidentiteCreateModel(BaseModel):
...
class Naturepieceidentite(BaseModel):
    uid: uuid.UUID
    libelle: str
...
class GetNaturepieceidentiteModel(BaseModel):
    naturepieceidentite_uid: str


class NaturepieceidentiteUpdateModel(BaseModel):
    uid: uuid.UUID
    libelle: str
    code: str | None = Field(default=None, max_length=255)
    cree_le: datetime | None = Field(default=None)
...
class NaturepieceidentiteResponse(BaseModel):
...
class MessageAllResponse(BaseModel, Generic[T]):
    ...
    message: str
    success: bool = True
    data: T | None | List[NaturepieceidentiteResponse] = None

class MessageResponse(BaseModel, Generic[T]):
    code: int
    message: str
    success: bool = True
    data: T | None | NaturepieceidentiteResponse = None
